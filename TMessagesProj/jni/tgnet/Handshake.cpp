    ...
    if (authNonce->isEqualTo(result->nonce.get())) {
        ...
        if (currentDatacenter->isCdnDatacenter) {
            ...
            if (serverPublicKeys.empty()) {
                // FloppaMeet is a private Telegram-compatible service. Do not
                // retain Telegram's public key here: if a stale Telegram DC
                // address is ever selected, its handshake must be rejected.
